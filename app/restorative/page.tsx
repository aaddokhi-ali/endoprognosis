...
interface RestorativeData {
  ...
  perio: string;
  fullResult?: any;
  sourceRoute?: string; // "/predictor/result" | "/endodecide/result"
}
...
function buildRecommendation(
  ...
  const usableWalls   = intactWalls + moderateWalls;
  ...
  if (hasBruxismRisk) {
    contextualWarnings.push(
      `Prosthodontic complexity noted. Abu-Awwad 2025 found parafunctional habits carry a 12.8× increased failure risk — full-coverage crown is preferred, and an occlusal splint should be considered post-restoration.`
    );
  }
  if (ferruleInsufficient && percent < 50) {
    contextualWarnings.push(
      `Ferrule absent or present on ≤2 walls. Juloski 2012: if no ferrule can be achieved by any means, poor clinical outcome is very likely. Discuss orthodontic extrusion, surgical crown lengthening, or extraction + implant.`
    );
  }
  ...
  if (toothType === "anterior") {
    if (percent >= 75) {
      ...
      if (ferruleAdequate) {
        ...
        ferruleNote = "Ferrule compromised on 1 wall. Consider orthodontic extrusion to gain 1.5–2 mm ferrule height before crown preparation (Juloski 2012).";
        urgencyLevel = "amber";
      } else {
        ferruleNote = "Ferrule insufficient (≤2 walls). Orthodontic extrusion or surgical crown lengthening required before definitive restoration.";
        urgencyLevel = "red";
      ...
      clinicianSteps = [
        ...
        ferruleAdequate ? "Proceed with fiber post placement in the largest straight canal." : "Achieve ≥1.5 mm ferrule via extrusion or crown lengthening before post placement.",
      ...
      rationale = "Severe structure loss — ferrule achievement is the critical decision point. Ferrari et al. 2007: all debonding failures occurred in teeth with <2 mm coronal dentin and <2 residual walls.";
      ...
      if (ferruleInsufficient) {
        ferruleNote = "Insufficient ferrule — orthodontic extrusion or crown lengthening mandatory before restoration.";
        urgencyLevel = "red";
        ...
        ferruleNote = ferruleAdequate ? "Adequate ferrule present. Proceed with 1.5–2 mm circumferential preparation (Juloski 2012)." : "Partial ferrule — achieve full circumferential ≥1.5 mm where possible.";
      }
      clinicianSteps = [
      ...
      urgencyLevel = "red";
      escalationNote = "Present extraction + implant-supported crown as primary option. If patient elects tooth retention: orthodontic extrusion or surgical crown lengthening is mandatory before restoration.";
      clinicianSteps = [
    ...
    if (percent >= 75 && usableWalls >= 3 && !hasBruxismRisk) {
      ...
      rationale = "High remaining structure with ≥3 usable walls. Wang et al. 2022 meta-analysis: onlays/partial crowns statistically equivalent to full crowns at 1 year (OR 0.55) and 3 years (OR 0.65).";
      postNote  = "No post required — pulp chamber and remaining walls provide adequate core retention (Schwartz 2004).";
      ferruleNote = ferruleAdequate ? "Adequate structure — ferrule preparation not required for onlay." : "Minor ferrule compromise — still suitable for onlay if ≥3 walls usable.";
      urgencyLevel = "green";
      clinicianSteps = [
      ...
      primary   = hasBruxismRisk ? "Full-coverage ceramic crown (preferred in bruxers/parafunction risk)" : "Ceramic onlay with full cuspal coverage OR full-coverage ceramic crown";
      rationale = hasBruxismRisk ? "Parafunctional habit risk detected. Abu-Awwad 2025: parafunction carries HR 12.8 for restoration failure. Full crown provides superior load distribution." : "Moderate structure loss with cusp involvement. Schwartz & Robbins 2004: cuspal coverage provides 6× survival advantage.";
      ...
      ferruleNote = ferruleAdequate ? "Adequate ferrule — proceed with preparation ensuring ≥1.5 mm at all margins." : ferruleCompromised ? "Ferrule compromised on 1 wall — acceptable for full crown. Aim for ≥1.5 mm on remaining walls." : "Insufficient ferrule — consider extrusion or lengthening to achieve ≥1.5 mm on ≥2 walls.";
      clinicianSteps = [
      ...
      primary   = "Fiber post + composite core → Full-coverage ceramic crown";
      rationale = "Severe structure loss requiring core support. Ferrari et al. 2007 (3-year follow-up): quartz-fiber post + crown achieved 96.2% survival in premolars vs 88.5% with cast post + crown.";
      ...
      ferruleNote = ferruleAdequate ? "Adequate ferrule — proceed. Ensure 1.5–2 mm vertical dentin above finish line on all walls (Juloski 2012)." : ferruleCompromised ? "Ferrule compromised on 1 wall. Achieve ≥1.5 mm where possible." : "Insufficient ferrule (≤2 walls). Crown lengthening or orthodontic extrusion required before definitive crown.";
      escalationNote = ferruleInsufficient ? "Refer for orthodontic extrusion or periodontal crown lengthening before crown preparation." : "";
      clinicianSteps = [
        "Evaluate ferrule on all 4 walls — address deficit before preparation.",
        "Place fiber post with dual-cure resin cement (4th-gen adhesive).",
      ...
      rationale = "Minimal remaining coronal structure. Al-Dabbagh 2021: endocrown 5-year survival 91.4% vs 98.3% conventional crown (difference not statistically significant, p>0.05). Endocrown avoids post space preparation.";
      postNote  = "Endocrown: no post required. If conventional crown elected: fiber post mandatory with extrusion/lengthening to achieve ferrule.";
      ferruleNote = "Endocrown uses pulp chamber depth for retention — pulp chamber depth ≥3 mm required (Al-Dabbagh 2021).";
      urgencyLevel = "amber";
    ...
    if (percent >= 75 && usableWalls >= 3 && !hasBruxismRisk) {
      ...
      rationale = "Abu-Awwad et al. 2025 (first RCT in ETT molars with minimal loss): direct composite and crowns showed statistically similar 3-year survival (76.7% vs 93.3%, p=0.061) when ≥3 axial walls are intact.";
      ...
      clinicianSteps = [
        ...
        "Monitor carefully — convert to crown if marginal breakdown detected.",
      ];
    } else if (percent >= 75 && (usableWalls < 3 || hasBruxismRisk)) {
      primary   = "Full-coverage ceramic or metal-ceramic crown";
      rationale = hasBruxismRisk ? "Despite adequate remaining structure, parafunctional habit risk requires full cuspal coverage. Abu-Awwad 2025: bruxism HR 12.8 for failure." : "Fewer than 3 usable walls despite adequate remaining percentage — direct composite lacks support.";
      postNote  = "Post not required — pulp chamber provides adequate retention.";
      urgencyLevel = "amber";
      ferruleNote = ferruleAdequate ? "Good ferrule — standard crown preparation with ≥1.5 mm margin." : "Partial ferrule — ensure maximum circumferential capture at crown margin.";
      clinicianSteps = [
      ...
      rationale = "Moderate structure loss with cusp involvement. Schwartz & Robbins 2004: endodontically treated posterior teeth with cuspal coverage are 6× more likely to survive than those with intracoronal restorations.";
      ...
      urgencyLevel = "amber";
      ferruleNote = ferruleAdequate ? "Adequate ferrule on all walls — standard crown preparation ensuring ≥1.5 mm supragingival dentin." : ferruleCompromised ? "Ferrule compromised on 1 wall — aim for ≥1.5 mm on remaining 3 walls." : "Insufficient ferrule — consider crown lengthening of compromised wall(s).";
      escalationNote = ferruleInsufficient ? "Crown lengthening of deficient wall(s) recommended before definitive crown." : "";
      clinicianSteps = [
        ...
        "Metal-ceramic crown for high-load posterior cases; all-ceramic acceptable with adequate occlusal clearance.",
      ...
      rationale = "Severe coronal structure loss. Schwartz 2004: single fiber post in largest straight canal. Juloski 2012: preservation of any coronal wall reduces failure risk.";
      ...
      urgencyLevel = ferruleInsufficient ? "red" : "amber";
      ferruleNote = ferruleAdequate ? "Adequate ferrule — capture ≥1.5 mm on all walls during crown prep (Juloski 2012)." : ferruleCompromised ? "Ferrule on 3 walls — acceptable; aim for ≥1.5 mm where present." : "Ferrule on ≤2 walls — crown lengthening or forced eruption needed.";
      escalationNote = ferruleInsufficient ? "Periodontal crown lengthening or forced eruption indicated before definitive crown. If neither feasible: discuss extraction + implant." : "";
      clinicianSteps = [
        ...
        "Dual-cure resin cement + 4th-generation adhesive.",
      ...
      rationale = "Minimal remaining coronal structure. Al-Dabbagh 2021 systematic review: endocrown 5-year survival 91.4% vs 98.3% conventional crown (not statistically significant, p>0.05). Endocrown avoids post space preparation, preserving radicular dentin.";
      postNote  = "Endocrown: no post (pulp chamber depth provides retention). If conventional crown chosen: fiber post mandatory — requires ferrule via crown lengthening/extrusion.";
      ferruleNote = "Endocrown retention relies on pulp chamber depth (minimum 3 mm) — verify before committing (Al-Dabbagh 2021).";
      urgencyLevel = "amber";
      ...
      clinicianSteps = [
        ...
        "If adequate: prepare for CAD-CAM endocrown (feldspathic CEREC or IPS e.max).",
  ...
  return { primary, rationale, ferruleNote, postNote, escalationNote, urgencyLevel, deferRestoration, contextualWarnings, clinicianSteps };
}
...
export default function RestorativeRecommendation() {
  ...
  const [sourceRoute, setSourceRoute] = useState("/predictor/result");
  ...
  useEffect(() => {
    ...
    if (parsedData.sourceRoute) setSourceRoute(parsedData.sourceRoute);
      ...
      <div className="min-h-screen relative overflow-hidden bg-[#0a1428]">
        ...
        <div className="relative z-10 flex min-h-screen items-center justify-center px-4">
          <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-10 text-center max-w-md w-full">
            ...
            <p className="text-gray-300 mb-8">Please go back and calculate the case first.</p>
            <Link href="/endodecide" className="block bg-[#10b981] hover:bg-[#0ea76e] text-black font-bold py-4 rounded-2xl transition mb-3">
              Go to EndoDecide
            </Link>
            <Link href="/predictor" className="block bg-white/8 hover:bg-white/15 border border-white/15 text-white font-semibold py-4 rounded-2xl transition">
              Go to Predictor
            </Link>
          </div>
        </div>
  ...
  const isEndoDecide  = sourceRoute === "/endodecide/result";
  const backLabel     = isEndoDecide ? "Back to EndoDecide Result" : "Back to Predictor Result";
  const accentColor   = isEndoDecide
    ? (data.fullResult?.urgency === "high" ? "#ef4444" : data.fullResult?.urgency === "medium" ? "#f59e0b" : "#10b981")
    : "#10b981";
        ...
        <div className="max-w-4xl mx-auto space-y-5">
          ...
          <div className="text-center mb-2">
            ...
            <p className="text-gray-400">
              Tooth <span className="font-semibold" style={{ color: accentColor }}>#{data.toothNumber}</span>
              {" · "}<span className="text-gray-300">{toothLabel}</span>
              {" · "}<span className="font-semibold" style={{ color: accentColor }}>{data.remainingPercent}% remaining structure</span>
            </p>
            {/* Source tool badge */}
            <div className="flex justify-center mt-3">
              <span className="text-[10px] font-bold px-3 py-1.5 rounded-full border"
                ...
                {isEndoDecide ? "From EndoDecide" : "From Prognosis Predictor"}
              </span>
            </div>
          </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${
                  (data.ferrule?.wallsWithFerrule ?? 4) >= 4 ? "bg-emerald-500/15 text-emerald-400"
                  : (data.ferrule?.wallsWithFerrule ?? 4) >= 3 ? "bg-amber-500/15 text-amber-400"
              ...
              <p className={`text-xs font-semibold mb-2 ${
                (data.ferrule?.wallsWithFerrule ?? 4) >= 4 ? "text-emerald-400"
                : (data.ferrule?.wallsWithFerrule ?? 4) >= 3 ? "text-amber-400"
                : "text-red-400"
              }`}>
                {data.ferrule?.label || "—"}
              </p>
          ...
          <div className="bg-[#0d1a30] border border-white/10 rounded-2xl p-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
            ...
            <div className="grid sm:grid-cols-2 gap-2 text-xs text-gray-400">
              {[
                "Coronal seal must be achieved immediately after RCT (Schwartz 2004)",
                "Cuspal coverage for all posterior ETT (6× survival advantage — Schwartz 2004)",
                "Ferrule ≥1.5–2 mm doubles fracture resistance (Juloski 2012)",
                "Fiber posts preferred — restorable failure mode vs catastrophic (Ferrari 2007)",
                "Minimize radicular dentin removal — post space only when needed (Schwartz 2004)",
                "Quality of coronal restoration independently predicts periapical health (Ng 2007)",
              ].map((p, i) => (
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-2">
            ...
            <Link href="/home"
              className="flex-1 flex items-center justify-center gap-2 font-bold py-4 rounded-2xl text-sm transition-all text-black"
