...
import { signOut, sendEmailVerification, signInWithPopup, GoogleAuthProvider } from "firebase/auth";
...
// ── Google "G" logo (official colours, no external image needed) ──
function GoogleIcon() {
  return (
...
function LoginContent() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Google accounts are always verified — go straight to home
      router.replace("/home");
    } catch (err: any) {
      const msg = friendlyError(extractCode(err));
      if (msg) setError(msg);   // silent for cancelled-popup-request
    } finally {
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white/10 backdrop-blur-2xl border border-white/20 rounded-3xl p-6 sm:p-10 shadow-2xl">
            ...
            <button
              ...
              disabled={anyLoading}
              className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-50 disabled:bg-white/70 text-gray-800 font-semibold py-4 rounded-2xl text-base transition-all shadow-lg hover:shadow-xl mb-5"
            >
              {googleLoading ? (
            ...
            <div className="flex items-center gap-4 mb-5">
              ...
              <span className="text-gray-400 text-xs uppercase tracking-widest">or sign in with email</span>
              <div className="flex-1 h-px bg-white/20" />
            </div>
